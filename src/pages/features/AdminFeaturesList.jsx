...
import { getFeaturesWithTrashed, deleteFeature, restoreFeature, forceDeleteFeature } from '../../api/features_api'
...
function AdminFeaturesList() {
  ...
  const handleToggle = (id, isActive) => {
    if (isActive) {
      ...
      deleteFeature(id)
        .then(() => {
          fetchFeatures()
          setNotification({ id: Date.now(), type: "success", message: "Caracteristica desactivada correctament"})
        })
        .catch(err => {
          console.error(err)
          setNotification({ id: Date.now(), type: "error", message: "No s'ha pogut desactivar la caracteristica"})
        })
    } else {
      ...
      restoreFeature(id)
        .then(() => {
          fetchFeatures()
          setNotification({ id: Date.now(), type: "success", message: "Caracteristica restaurada correctament"})
        })
        .catch(err => {
          console.error(err)
          setNotification({ id: Date.now(), type: "error", message: "No s'ha pogut restaurar la caracteristica"})
        })
    }
  }
  ...
  const handleForceDelete = (id) => {
    forceDeleteFeature(id)
      .then(() => {
        fetchFeatures()
        setNotification({ id: Date.now(), type: "success", message: "Caracteristica eliminada permanentment"})
      })
      .catch(err => {
        console.error(err)
        setNotification({ id: Date.now(), type: "error", message: "No s'ha pogut eliminar permanentment la caracteristica"})
      })
  }
                ...
                <td className='border-base-300'>
                  <div className='flex justify-center'>
                    <input 
                      ...
                      onChange={() => handleToggle(feature.id, !feature.deleted_at)}
                    />
                  </div>
                </td>
                <td className='border-base-300'>
                  <div className='flex items-center justify-center gap-3'>
                    {feature.deleted_at ? (
                      <>
                        <ConfirmableModal title="Eliminar caracteristica permanentment"  message={`Segur que vols eliminar permanentment la caracteristica "${feature.value}"? Aquesta acció no es pot desfer.`}  onConfirm={() => handleForceDelete(feature.id)}>
